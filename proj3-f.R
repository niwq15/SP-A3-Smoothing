...
# Consider a model, y_i = f(x_i) + error_i, with the observed x, y data, f as an unknown smooth function, and error_i as a zero mean error term with some variance. 
# To estimate f, we will use evenly spaced B-spline basis functions which consist of 2 parts that need to be estimated, basis functions and the coefficients. 
# The basis functions are bell shaped functions, and they are well constructed that the evaluations at any x value within the range of the data sum to 1. 
# To avoid over fitting, we use a smoothing penalty to encourage the coefficients corresponding to the neighbouring basis functions to vary smoothly from one to the next. 
# The model is estimated by penalized least squares and we choose a smoothing parameter, lamb, which minimized the generalized cross validation criterion (GCV). 
# During the process we also need to consider the effective degrees of freedom (edf)
# To obtain the best lamb, we mainly need to use the knots (the vector to to set up the B-spline basis), X (the basis), some matrices such as Q and R (can be obtained 
# from the QR decomposition X = QR), U and lambda (obtained from eigen-decomposition), D (will be defined later), I (an identity matrix), S (covariance matrix), 
# edf (the effective degrees of freedom), sig2 (the residual variance), coef (the coefficients), and fitted (the fitted values). 
...
# The following are some useful formulas to calculate those parameters/variables. 1. S = sig2*((X^T)X+lamb*(D^T)D)^-1 = (R^-1)U((I+lamb*lambda)^-1)(U^T)(R^-T)); 
# 2. coef = (((X^T)X+lamb*(D^T)D)^-1)(X^T)y = (R^-1)U((I+lamb*lambda)^-1)(U^T)(Q^T)y = sig2*(((X^T)X+lamb*(D^T)D)^-1)(Q^T)y; 3. UΛ(U^T) = (R^−T)(D^T)D(R^−1); 
# 4. sig2 = (||y-fitted||^2)/(n-edf); 5. edf = tr{(I+lamb*lambda)^-1} = tr{(((X^T)X+lamb*(D^T)D)^-1)(X^T)X}; 6. X = QR
# More details can be seen in Professor Simon Wood's notes, "Basis Penalty Smoothers"

# Firstly, we will show a 'pspline' function which returns an object (a list) defining the best fit spline smoother, then create a 'print.pspline' function to report 
# some details such as edf and r-squared of the model fit which are obtained in the 'pspline' function, then use a 'predict.pspline' to make predictions for new x values
# (within the range of the original data) based on the smooth fit. Finally, a 'plot.pspline' function is created to give 3 plots: Plot1 shows the original x,y data and the 
# overlaid estimated smooth function, together with a 95% credible intervals for the smooth; Plot2 gives the plot of the model residuals against the fitted values; Plot3 is 
...
## The function uses a splineDesign function to set up the basis (X matrix), then adopts QR decomposition (components: R, Q) and eigen decomposition (components: U, lambda),
## then use a for-loop to calculate the values of those variables corresponsing to different smoothing paramters, finally find the optimal smoothing parameter, lamb, 
## which minimizes the generalized cross validation criterion. 
## The 'pspline' function will return a list of elements, including x and y, bord, pord, D, X(the basis model matrix), U, R, I(an identity matrix), knots(the vector to set 
## up the B-spline basis), lamb (the best fit spline smoother), and the corresponding coef (the coefficients), fitted (the fitted values), sig2 (residual variance), edf, 
## gcv(generalized cross validation criterion), and S (the covariance matrix)
...
pspline <- function(x,y,k=20,logsp=c(-5,5),bord=3,pord=2,ngrid=100) {
  ...
  ## the GCV (gcv), the coefficient (coefval), and the fitted values (fittedval).
  ...
  for (i in 1:ngrid) { ## the number of lamb that needs to try is ngrid, 100
    ...
    fittedval[,i] <- X%*%coefval[,i] ## the fitted values
  ...
  ind <- which(gcv %in% sort(gcv)[1]) ## the 'sort' function helps to reorder the element from smallest to largest and hence the first element is the smallest
...
## The function uses the results of the smooth fit (obtained using the 'pspline' function), such as D, knots, and coef, to make predictions for new x values. 
## After extracting useful components from m, we firstly adopt 'splineDesign' to create a new model matrix based on the new x, then use the results of the smooth
## fit to obtain the covariance matrix for the coefficients, then use a computable efficiently method to calculate the standard errors.
...
## The function utilizes the components of m and the 'predict.pspline' funtion to make predictions on the fitted values, EY (a 2-item list of 'fit' and 'se'), 
## and uses the EY's 'fit' and 'se' values to calculate the lower and upper limits for the 95% credible intervals, ll and ul. Before plotting, we also reorder the data of 
## x, y, fit, ll and ul according to the x values. Then the model residuals will be calculated by the difference between the original y values and the predicted values.
## The residuals will used for both the second and the third plots. 
## The 'plot.pspline' function will return 3 plots: The first is a plot of the original x,y data, with the estimated smooth function overlaid as a line, together with 
## a 95% credible intervals for the smooth. The second is for the model residuals and the fitted values. The third is a qqplot of the model residuals. 
...
plot.pspline <- function(m) {
  ...
  names(out) <- c("x","ll","ul")
  plot(values[,1], values[,2], xlab="times",ylab="accel",pch=16,col='blue',main="Original x, y data with the estimated smooth function and 95% credible intervals")
  ...
  legend("bottomright", legend = c("original y values","fitted values","lower confidence limit","upper confidence limit"), col=c("blue","black","green","red"), lty=c(NA,1,2,2), pch=c(16,NA,NA,NA))
  
  ## The second plot: the model residuals against fitted values
  #EYo <- predict.pspline(m,x,se=TRUE)
  res <- values[,2] - values[,3] ## calculate the model residuals using the data after the reordering process
  plot(values[,3], res, xlab=" fitted values ",ylab="the model residuals ", pch=16, main="The model residuals against fitted values")
